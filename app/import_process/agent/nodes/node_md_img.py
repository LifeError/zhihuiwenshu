...
def step_1_get_content(state: ImportGraphState) -> Tuple[str, Path, Path]:
    ...
    if not state['md_content']:
        # 没有，再读取！有，证明是pdf节点解析过来的，已经给md_content赋值
...
def find_image_in_md_content(md_content, image_file, context_length: int = 100) -> Tuple[str, str] | None:
    """
    从md_content识别图片的上下文！
    约定上下文长度100
    :param md_content:
    ...
    """
    # 定义正则表达式  .*  .*?
    escape_image_file = re.escape(image_file)
    pattern = re.compile(r"!\[.*?]\(.*?" + escape_image_file + ".*?\)")
...
def step_3_generate_img_summaries(targets, stem) -> dict[str, str]:
    """
    获取图片的内容描述！ 利用视觉模型！
    ...
    for image_file, image_path, content_data in targets:
        ...
        # 2.2 准备提示词
        prompt = load_prompt(name='image_summary', root_folder=stem , image_content=content_data)
...
def step_4_upload_images_and_replace_md(summaries, targets, md_content, stem):
    ...
    # 需要DeleteObject
    # 1.2 调用方法进行删除即可
    errors: Iterator[DeleteError] = minio_client.remove_objects(minio_config.bucket_name, delete_object_list)
    ...
    for image_file, image_path, _ in targets:
        try:
            ...
            # 上传完毕以后记录
            # 图片地址 = 协议 + 端点 + 桶名 + 对象名  http://127.0.0.1:9000/ 桶名 / 对象名
    ...
    logger.info(f"已经完成图片上传，本次上传了：{count}个图片")
    # 3. md中图片的替换即可
    # summaries = 图片名: 描述
    ...
    if image_infos:
        ...
        for image_file, (summary, url) in image_infos.items():
            ...
            escape_image_file = re.escape(image_file)
            rep = re.compile(r"!\[.*?]\(.*?" + escape_image_file + ".*?\)")
            # md_content = rep.sub(f"![{summary}]({url})", md_content)
            # 清理summary中的特殊字符，避免Markdown语法冲突
            safe_summary = summary.replace('\n', ' ').replace('\r', ' ').replace('|', '｜').replace('[', '【').replace(']', '】')
            # 使用re.escape对替换内容中的特殊字符进行转义处理
            # 但由于我们需要保留![]()的Markdown语法，所以手动构建替换字符串
            # 关键：使用lambda函数作为re.sub的第二个参数，这样返回值不会被当作正则模板解析
            def replace_func(match):
                return f"![{safe_summary}]({url})"
            md_content = rep.sub(replace_func, md_content)
...
def step_5_replace_md_and_save(new_md_content, md_path_obj):
    ...
    logger.info(f"已经完成md文件的保存，新的md文件保存在：{new_md_path_str}")
