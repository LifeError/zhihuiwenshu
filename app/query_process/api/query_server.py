...
# 定义fastapi对象
app = FastAPI(title="query service", description="智慧问数查询服务！")
...
async def history(session_id: str,limit: int = 10):
    ...
    # 处理 ObjectId 序列化问题：将 _id 从 ObjectId 转为字符串
    processed_chats = []
    for chat in chats:
        # 复制字典并转换 _id
        chat_dict = dict(chat)
        ...
        processed_chats.append(chat_dict)
    # chat mongodb _id -> ObjectId -> 不能直接序列化 json
    # items = []
    # for chat in chats:
    #     items.append(chat)
    logger.info(f"查询历史对话，session_id = {session_id}成功！查询数据为：{processed_chats}")
    return {
        "session_id":session_id,
        "items": processed_chats
    }
