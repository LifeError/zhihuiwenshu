...
def step_2_upload_and_poll(pdf_path_obj) -> str:
    """
    将pdf文件使用mineru解析，并且获取md对应的下载俩地址
    :param pdf_path_obj: 上传解析pdf文件的 path 对象
    :return: str -> url ， mineru解析md文件zip压缩包的下载地址
    """
    # 1.申请上传解析的地址
    ...
    if response.status_code != 200 or response.json()['code'] != 0:
        logger.error(f"[step_2_upload_and_poll]使用minerU解析pdf文件发生了异常，请检查输入文件路径是否正确！")
        raise RuntimeError(f"[step_2_upload_and_poll]使用minerU解析pdf文件发生了异常，请检查输入文件路径是否正确！")

    uploaded_url = response.json()['data']['file_urls'][0]  # 从这个地址上传文件
    batch_id = response.json()['data']['batch_id']

    # 2.将文件上传到对应的解析地址
    # 使用Put请求，将pdf_path_obj上传到uploaded_url地址即可！
    ...
    # 3.轮询获取解析结果
    # 循环获取！确保获取到结果，再先后执行！！
    # 设计一个循环，3秒获取一次！ 最多等待5分钟300秒 -> 300页pdf
    url = f"{mineru_config.base_url}/extract-results/batch/{batch_id}"
    timeout_seconds = 300   # 1s -> 1页pdf
    poll_interval = 3   # 请求间隔时间3秒
...
def step_3_download_and_extract(zip_url, local_dir_obj, stem):
    ...
    with zipfile.ZipFile(zip_save_path, 'r') as zip_save_obj:
        # 调用对象的解压方法进行解压即可 参数：解压的目标文件夹  output/二狗子
        zip_save_obj.extractall(extract_target_dir)

    # 5. 返回md文件的地址
    # 解压以后得文件的文件名 可能叫 文件.md 低版本 也可能叫 full.md  image存在专门的images文件夹
