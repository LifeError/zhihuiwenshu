...
def step_2_split_by_title(md_content, file_title):
    ...
    # 1.3 定义临时存储变量 current_title = str | current_lines = [] | title_count = 0 存储了多少块 标题 多少行
...
def merge_short_sections(final_sections, min_length):
    ...
    for section in final_sections:
        ...
        if pre_section is None:
            pre_section = section
            continue
        # current_section 是第一次（上一次）  section 本次（当前块）
        is_current_short = len(pre_section.get("content")) < min_length # 判断上一次是不是短块
        ...
        if is_current_short and is_same_parent_title:
            # 上一次是短块，本次是短块，并且是同一个parent_title
            # 又短 又是同一个parent （合并）
            pre_section["content"] += "\n\n" + section.get("content")
            pre_section['part'] = section.get("part")  # 1 <- 2
        else:
            # 上一次不是短块，本次不是短块，或者不是同一个parent_title
...
def step_3_refine_chunks(sections, max_length,min_length):
    """
    做内容精细切割！
        1. 超过了 DEFAULT_MAX_CONTENT_LENGTH 块，要做切割！ （parent_title | part ）
        2. 小于了MIN_CONTENT_LENGTH块，要合并结果！ （同一个parent_title)
    :param sections:
    :param max_length: DEFAULT_MAX_CONTENT_LENGTH
    :param min_length: MIN_CONTENT_LENGTH
    :return: sections
    """
