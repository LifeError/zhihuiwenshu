...
def step_4_query_milvus_item_names(item_names):
    ...
    # 2. 将item_name 转成向量（稀疏和稠密）
    embeddings = generate_embeddings(item_names)
    # 3. 混合查询（创建稠密和稀疏的的AnnSearchRequest || 设置权重重排 || 进行混合查询）
...
def step_5_confirmed_and_optional_item_name(query_milvus_results):
    ...
    思路： 1. 循环处理每个item_name列表和分
          2. 高分 只要1个
          3. 可选 可以要2
          4. 不区分extracted:item_name 装到对应的 确认或者可选集合中
    """
    #1. 准备两个列表 确认 可选的
        ...
        if len(high_score_matches) > 1:
            ...
            for item in high_score_matches:
                if item.get("item_name") == extracted_name:
                    ...
                    break
            # 没有相同获取分数最高的
            if not save_name_item:
                save_name_item = high_score_matches[0]
            confirmed_item_names.append(save_name_item.get("item_name"))
            continue
...
def node_item_name_confirm(state):
    ...
    #          answer : 可选的答案
        1. 获取历史条件记录（作为依据）
        2. 保存当前次的聊天记录
        3. 利用模型llm -> 1. 提取item_names  2.重写提问内容
